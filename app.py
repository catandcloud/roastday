...
# End the order query at the current time
...
for order in order_results['orders']:
	...
	for item in order['items']:
		...
		# Init the sku coffees dict the first time
...
# Open up and read the csv file template
...
roast_date_file = '%s_Roast.csv' % arrow.utcnow().to('US/Pacific').format('YYYY-MM-DD')
write_file = roast_dir + '/' + roast_date_file
with open(write_file, 'w') as csvfile:
	fieldnames = ['SKU', 'Coffee', '10oz', '1lb', 'KILO', '5lb', 'ROASTED LBS', 'GREEN LBS']
